...
(function () {
  ...
  function parse(patterns, incoming, output) {
    ...
    if (!match) {
      ...
      var replacement =
        '<span class="' + matchType + '">' + match[1] + "</span>";
  ...
  function clean(text) {
    ...
    var dedented = textArray
      .map(function (string, index) {
        return !string || /^\s+$/.test(string)
          ? string
          : string.replace(spaceToCut[0], "");
      })
      .join("\n");
  ...
  function highlight(config) {
    ...
    Array.prototype.slice
      ...
      .forEach(function (block) {
        var patterns =
          (typeof config.patterns === "function"
            ? config.patterns(block)
            : config.patterns) || {};
